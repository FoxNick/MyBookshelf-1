...
class ChapterProvider {
    ...
    private TxtChapter loadPageList(BookChapterBean chapter, @NonNull String content) {
        ...
        if (pageLoader.book.isAudio()) {
            ...
            page.setTitle(contentHelper.replaceContent(this.pageLoader.book.getBookInfoBean().getName(), this.pageLoader.book.getTag(), chapter.getDurChapterName(), true,true));
            page.addLine(contentHelper.replaceContent(this.pageLoader.book.getBookInfoBean().getName(), this.pageLoader.book.getTag(), chapter.getDurChapterName(), true,true));
        ...
        //获取数据的时候做了替换
        content = contentHelper.replaceContent(pageLoader.book.getBookInfoBean().getName(), pageLoader.book.getTag(), content, pageLoader.book.getReplaceEnable(),false);
        String[] allLine = content.split("\n");
        List<String> lines = new ArrayList<>();
        List<TxtLine> txtLists = new ArrayList<>();//记录每个字的位置 //pzl
        int rHeight = pageLoader.mVisibleHeight - pageLoader.contentMarginHeight * 2;//写文字的高度
        int titleLinesCount = 0;
        boolean showTitle = pageLoader.readBookControl.getShowTitle(); // 是否展示标题
        String paragraph = null;
        if (showTitle) {
            paragraph = contentHelper.replaceContent(pageLoader.book.getBookInfoBean().getName(), pageLoader.book.getTag(), chapter.getDurChapterName(), true,true);
        ...
        while (showTitle || i < allLine.length) {
            ...
            //判断是第一页
            if(i ==1 && showTitle){
                rHeight -= pageLoader.mFirstPageMarginTop ; //上面留白
                rHeight -= pageLoader.mFirstPageMarginButtom ; //下面也留白
            }
            ...
            while (paragraph.length() > 0) {
                ...
                if (rHeight <= 0) {
                    ...
                    page.addLines(lines);
                    page.txtLists = new ArrayList<TxtLine>(txtLists);//pzl
                ...
                if (!subStr.equals("\n")) {
                    ...
                    txtList.CharsData = new ArrayList<TxtChar>();
                    for (int y = 0, size = cs.length; y < size; y++) {
                        String mesasrustr = String.valueOf(cs[y]);
                        ...
                        txtChar.chardata = cs[y];
                        txtChar.charWidth = charwidth;//字宽
        ...
        if (lines.size() != 0) {
            ...
            page.setTitle(chapter.getDurChapterName());
            page.addLines(lines);
            page.txtLists = new ArrayList<TxtLine>(txtLists);
            page.setTitleLines(titleLinesCount);
            txtChapter.addPage(page);
            addTxtPageLength(txtChapter, page.getContent().length());
            //重置Lines
